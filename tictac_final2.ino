...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Handle game mode or move input
  if (message == "1" || message == "2" || message == "3") {
    receivedMove = message.toInt();  // Menu input
  } else if (message.toInt() >= 1 && message.toInt() <= 9) {
    receivedMove = message.toInt();  // Move input
  }
}
...
void pvp() {
  ...
  while (!gameOver && moves < 9) {
    ...
    if (checkWin()) {
      ...
      mqttClient.publish("move", result); // Notify TUI
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print(result);
      lcd.setCursor(0, 1);
    ...
    if (moves == 9) {
      mqttClient.publish("move", "Draw"); // Notify TUI
      lcd.clear();
      lcd.setCursor(0, 0);
